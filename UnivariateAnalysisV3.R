...
setwd("C:\\Users\\lkgupta\\OneDrive - Syracuse University\\IntroToDS\\Project")
#setwd("C:\\Users\\The Lakshya\\Documents\\Academics\\IntroToDataScience\\Project")
...
npsCalculatorFilter <- function(df,columnName,ini=3,fin=8,Top=10){
  ...
  #View(NPSCount)
  NPSCount <-NPSCount[order(-NPSCount$TotalObservations),]
  NPSCount<- NPSCount[1:Top,]
  ggplot(NPSCount, aes(x=reorder(Group.1,npsScore), y=npsScore,size=TotalObservations)) +
    geom_point(alpha=0.3) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
    scale_size_continuous(range = c(ini, fin)) +
...
#type of Travel
npsCalculatorFilter(df,'Destination.State',6,15,10)
npsCalculator(df,'Type.of.Travel',5,10)
